...
class ReportController extends Controller
{
    ...
    public function generalCommercials(Request $request)
    {
        ...
        $ordersByDay = OrdersSales::whereBetween('sale_date', [$start, $end])
        ...
        $avgTicket = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->avg('total');

        // 3. Usuarios activos (nuevos vs recurrentes) usando buyer_id
        $buyersIds = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->pluck('buyer_id')
            ->unique();
        ...
        $businessWithOrders = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->distinct('busines_id')
            ->count('busines_id');
        ...
        // 5. Top tiendas
        $topBusinesses = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->selectRaw('busines_id, COUNT(*) as total_orders, SUM(total) as total_amount')
            ->groupBy('busines_id')
            ->orderByDesc('total_orders')
            ->with('business') // asegúrate de que OrdersSales tenga relación business()
            ->take(10)
            ->get();

        // 6. Top categorías
        $topCategories = Category::selectRaw('category.name, COUNT(orderssales_detail.orderDet_id) as total_items')
            ...
            ->whereBetween('orderssales.sale_date', [$start, $end])
        ...
        $repeatCustomers = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->select('buyer_id')
            ->groupBy('buyer_id')
            ->havingRaw('COUNT(*) > 1')
            ->count();

        $totalCustomers = OrdersSales::whereBetween('sale_date', [$start, $end])
            ->distinct('buyer_id')
            ->count('buyer_id');
    ...
    public function OperationalCommercials(Request $request)
    {
        ...
        if ($filtro === 'negocios') {
            ...
            $orders = OrdersSales::with(['address', 'business'])
                ->whereBetween('sale_date', [$start, $end])
                ->when($business_id, fn($q) => $q->where('busines_id', $business_id))
                ->when($domiciliary_id, fn($q) => $q->where('domiciliary_id', $domiciliary_id))
                ->get();
            ...
            $coordenadas = $orders->map(fn($o) => [
                ...
                'label' => 'Pedido ' . $o->orderSales_id . ($o->business ? ' - ' . $o->business->name : ''),
                'type' => 'pedido',
                'id' => $o->orderSales_id
