...
class ReviewController extends Controller
{
    public function store(Request $request)
    {
        ...
        $user = User::select('id', 'roles')->where('user_id', $request->user_id)->firstOrFail();
        ...
        if ($reviewCreated && $orderId) {
            OrderSale::where('order_sale_id', $orderId)->update(['has_review' => 1]);
        }

        // 🔹 Cargar solo las relaciones existentes según tipo de review
        $relations = [];
        if ($review instanceof \App\Models\BusinessReview) {
            $relations = ['business', 'buyer.user'];
        } elseif ($review instanceof \App\Models\DomiciliaryReview) {
            $relations = ['domiciliary', 'buyer.user'];
        } elseif ($review instanceof \App\Models\UserReview) {
        ...
        $formatted = $reviews->map(function ($review) {
            return [
                ...
                'business' => [
                    ...
                    'razon_social' => $review->business->legal_name,
        ...
        $formatted = $reviews->map(function ($review) {
            return [
                ...
                'business' => $review->business ? [
                    ...
                    'qualification' => $review->business->qualification,
                    'razon_social' => $review->business->legal_name,
                    'legal_name' => $review->business->legal_name,
                    'NIT' => $review->business->NIT,
                    'logo' => $review->business->logo,
                    'city' => $review->business->city,
                    'state' => $review->business->state,
    ...
    public function createBusinessReview(Request $request)
    {
        ...
        try {
            ...
            Business::where('business_id', $review->business_id)
    ...
    public function deleteBusinessReview(Request $request)
    {
        ...
        try {
            ...
            $average = BusinessReview::where('business_id', $business_id)
                ->where('state', true)
                ->avg('qualification');
        ...
        $formatted = $reviews->map(function ($review) {
            return [
                ...
                'domiciliary' => [
                    ...
                    'user' => [
                        'user_id' => $review->domiciliary->user->id,
