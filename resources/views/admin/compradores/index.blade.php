                ...
                <tbody>
                    @foreach ($buyers as $buyer)
                        <tr>
                            ...
                            <td>
                                <span class="badge {{ $buyer->belongs_to_complex ? 'bg-info' : 'bg-light text-dark' }}">
                                    {{ $buyer->belongs_to_complex ? 'Sí' : 'No' }}
                                </span>
                            </td>
                            <td>
                                <a href="{{ route('admin.compradores.edit', $buyer->buyer_id) }}" class="btn btn-sm btn-warning me-1">
                                    <i class="fas fa-edit"></i> Editar
                                </a>
                                <form action="{{ route('admin.compradores.destroy', $buyer->buyer_id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
